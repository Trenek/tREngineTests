...
void main()
{
    ...
    #endif
    vec2 uv = (2.*(fragCoord+vec2(x,y)));
    
    const float shutter_speed = .25; // for motion blur
	//float dt = fract(texture(iChannel0,float(AA)*(fragCoord+vec2(x,y))/iChannelResolution[0].xy).r+ubo.iTime)*shutter_speed;
    float dt = fract(hash21(float(AA)*(fragCoord+vec2(x,y))))*shutter_speed;
    jTime = 0.0;
