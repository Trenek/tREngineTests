...
void recTest(struct EngineCore *engine, enum state *state) {
    ...
    struct ResourceManager *commandQueue = findResource(&engine->resource, REC_COMMAND_QUEUE);
    ...
    struct CommandQueue *graphics = findResource(commandQueue, REC_COMMAND_QUEUE_GRAPHICS);
