...
static void createScreens(struct EngineCore *engine) {
    ...
    addResource(screenData, OBJ_SCREEN_1, createRenderPassObj((struct renderPassBuilder){
...
static void createCommandQueues(struct EngineCore *engine) {
    ...
    addResource(queueData, OBJ_COMMAND_QUEUE_GRAPHICS, createCommandQueue(&engine->graphics, "Graphics Buffer"), destroyCommandQueue);
