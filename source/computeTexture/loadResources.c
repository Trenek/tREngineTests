...
static void addObjectLayout(struct EngineCore *this) {
    ...
    addResource(objectLayoutData, COMP_TEXT_OBJECT_LAYOUT_COMPUTE, createDescriptorSetLayoutObj(3, (VkDescriptorSetLayoutBinding[]) {
        [0] = {
            .binding = 0,
            .descriptorCount = 1,
...
static void createMemoryBuffers(struct EngineCore *this) {
    ...
    addResource(data, COMP_TEXT_BUFFER_DATA_MAPPED, uniformMapped, free);
    addResource(data, COMP_TEXT_BUFFER_DATA_DATA, bd, destroyBufferData);

    addResource(&this->resource, COMP_TEXT_BUFFER_DATA, data, cleanupResourceManager);
}

static void createDescriptorSetss(struct EngineCore *this) {
    struct descriptorSetLayout *computeLayout = findResource(findResource(&this->resource, COMP_TEXT_OBJECT_LAYOUT), COMP_TEXT_OBJECT_LAYOUT_COMPUTE);
    struct BufferData *bd = findResource(findResource(&this->resource, COMP_TEXT_BUFFER_DATA), COMP_TEXT_BUFFER_DATA_DATA);
    addResource(&this->resource, COMP_TEXT_DESCRIPTOR, createDescriptorSetsObj(&this->graphics, &(struct DescriptorObjBuilder) {
    ...
    struct DescriptorObj *descriptor = findResource(&this->resource, COMP_TEXT_DESCRIPTOR);
...
static void createGraphicPipelineLayouts(struct EngineCore *this) {
    ...
    struct descriptorSetLayout *compLayout = findResource(findResource(&this->resource, COMP_TEXT_OBJECT_LAYOUT), COMP_TEXT_OBJECT_LAYOUT_COMPUTE);

    addResource(graphicPipelinesData, COMP_TEXT_GRAPHIC_PIPELINE_LAYOUT_COMP, createGraphicPipelineLayout((struct graphicsPipelineLayoutBuilder) {
        .descriptorSetLayout = (VkDescriptorSetLayout []){
            compLayout->descriptorSetLayout,
        },
        .qDescriptorSetLayout = 1,
    }, &this->graphics), destroyPipelineLayoutObj);
    addResource(graphicPipelinesData, COMP_TEXT_GRAPHIC_PIPELINE_LAYOUT_PURE, createGraphicPipelineLayout((struct graphicsPipelineLayoutBuilder) {
    }, &this->graphics), destroyPipelineLayoutObj);

    addResource(&this->resource, COMP_TEXT_GRAPHIC_PIPELINE_LAYOUTS, graphicPipelinesData, cleanupResourceManager);
}
...
static void createGraphicPipelines(struct EngineCore *this) {
    ...
    struct graphicsPipelineLayout *pipelineCompLayout = findResource(findResource(&this->resource, COMP_TEXT_GRAPHIC_PIPELINE_LAYOUTS), COMP_TEXT_GRAPHIC_PIPELINE_LAYOUT_COMP);
    struct graphicsPipelineLayout *pipelinePureLayout = findResource(findResource(&this->resource, COMP_TEXT_GRAPHIC_PIPELINE_LAYOUTS), COMP_TEXT_GRAPHIC_PIPELINE_LAYOUT_PURE);
    ...
    addResource(graphicPipelinesData, COMP_TEXT_GRAPHIC_PIPELINE_PURE, createGraphicsPipelineObj((struct GraphicsPipelineBuilder) {
    ...
    addResource(graphicPipelinesData, COMP_TEXT_GRAPHIC_PIPELINE_COMP, createComputePipelineObj((struct ComputePipelineBuilder) {
...
static void addEntities(struct EngineCore *this) {
    ...
    *b = (struct Entity) {
    ...
    addResource(entityData, COMP_TEXT_DRAWCALLS_1, a, free);
    addResource(entityData, COMP_TEXT_ENTITIES_1, b, free);
