...
void compTextTest(struct EngineCore *engine, enum state *state) {
    ...
    struct renderPassObj *renderPass[] = {
        findResource(screenData, COMP_TEXT_SCREEN_2),
    };
    ...
    struct UniformBufferObject **time = findResource(findResource(&engine->resource, COMP_TEXT_BUFFER_DATA), COMP_TEXT_BUFFER_DATA_MAPPED);
    ...
    struct DrawCall2 *dc = findResource(findResource(&engine->resource, COMP_TEXT_ENTITIES), COMP_TEXT_DRAWCALLS_1);
